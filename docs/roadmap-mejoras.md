...
## 1. Más comparables y más precisos (coches.net)
...
## 2. Autorelleno desde URL del anuncio
...
## 3. Persistencia de análisis
- Integrar el trabajo del PR #8 (Upstash Redis en Vercel / ficheros en local).
- Chat y `/vehicle/[id]` dejan de romperse tras cold start.

## 4. Corpus RAG técnico
- Integrar el trabajo del PR #7 (packs de conocimiento + chat orientado).
- Mejora “¿es fiable?” / averías / preguntas al vendedor.

## Criterio de hecho por punto
1. ≥12–24 comparables en modelos comunes tras filtrar, o nota explícita de muestra baja.
2. Pegar URL de coches.net rellena al menos marca/modelo/año/combustible (y precio/km si el anuncio aparece en listados).
3. Tras `analyze`, `GET /api/vehicle/[id]` responde en un proceso nuevo.
4. Chat/fiabilidad usan corpus ampliado; `rag:eval` o build OK.
